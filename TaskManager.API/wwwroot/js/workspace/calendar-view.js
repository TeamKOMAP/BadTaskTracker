...
export const createCalendarViewController = (deps) => {
  ...
  const getCalendarBucketId = typeof deps?.getCalendarBucketId === "function" ? deps.getCalendarBucketId : () => "today";
  const toPriorityValue = typeof deps?.toPriorityValue === "function" ? deps.toPriorityValue : (value) => Number(value) || 2;

  const compareCalendarTasks = (a, b) => {
  ...
  const upsertTaskInCalendar = (taskData) => {
    ...
    const targetBucketId = getCalendarBucketId(taskData);
    ...
    affectedGroups.forEach((group) => ensureCalendarGroupUiState(group));
    return true;
  };

  const removeTaskFromCalendar = (taskId) => {
    if (!calendarLayout) return false;
  ...
  const renderCalendarView = (tasks) => {
    ...
    const lists = new Map(CALENDAR_BUCKETS.map((bucket) => [bucket.id, []]));
    (Array.isArray(tasks) ? tasks : []).forEach((task) => {
      const bucketId = getCalendarBucketId(task);
    ...
    CALENDAR_BUCKETS.forEach((bucket) => {
